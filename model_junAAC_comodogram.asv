...
for si = 1 :  totalsub
    ...
    corr_rho_theta_gamma = zeros(length(freqBins),length(freqBins),length(index));
    z_corr_rho_theta_gamma = zeros(length(freqBins),length(freqBins),length(index));
    tic
    for i =lowTickIdx(1):lowTickIdx(end)  % 2:25 Hz
        rel_power_theta_band = squeeze(abs_power(i, :, theta_region)); % 40x15
        for j = highTickIdx(1):highTickIdx(end) % 30:80 Hz
            rel_power_gamma_band = squeeze(abs_power(j, :, gamma_region)); % 40x15
            % theta - gamma
            corr_rho_theta_gamma(i,j,:) = restingAnalysisModule.SpearmanCorrelation(...
                rel_power_theta_band, rel_power_gamma_band); % input:40trials*15regions (both)
            % inside theta band average within region, output: 1:15(regions)
            z_corr_rho_theta_gamma(i,j,:) = restingAnalysisModule.FisherZtrans(...
                corr_rho_theta_gamma(i,j,:));
        end
    end
    ...
    for pi = 1 : numel(PSDType)
        ...
        % create band indexes and global cluster for correlation
    globalaac = @(globalPower, localNodePower) ...
        corr(globalPower, localNodePower, 'Type', 'Spearman');
aac = [];
    bandname = []; bandindex =[]; bandpower =[]; cluster =[];
    for pi = 1 : numel(PSDType)
        
        % 281x161x68
        PSD = gather(PSDArray.(PSDType{pi})); %#ok<*NASGU> 

        for lowbandi = lowTickIdx(1):lowTickIdx(end)
            for highbandi =  highTickIdx(1):highTickIdx(end)
                fprintf("%2.2f Hz with %2.2f Hz\n", freqList(lowbandi), freqList(highbandi));
   
                trial_pow_low = squeeze(PSD(lowbandi, :, :));
                cluster_indices = 1 : numel(chanlocs);  % replace with any channel index, i.e. network indexes
                cluster_low =  squeeze(mean(trial_pow_low(:,cluster_indices),2));
                globalnode = repmat(cluster_low, [1 size(trial_pow_low,2)]);

                trial_pow_high = squeeze(PSD(highbandi, :, :));
                
                % 68 x 68 nodes
                aac.(PSDType{pi})(:,:,lowbandi) = globalaac(globalnode, trial_pow_high);
