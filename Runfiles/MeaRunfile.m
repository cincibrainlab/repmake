...
outdir.rest  = '/srv/BIGBUILD/Proj_Mea/';
indir.rest   = '/srv/BIGBUILD/Proj_Mea/';
indir.raw    = '/srv/onedrive/Carrie''s Mouse data/Carrie_MEA/S00_RAW/Carrie_MEA';

% Paradigm #1: Resting EEG (Rest)
fl.raw       = util_htpDirListing(indir.raw,'ext','.edf', 'subdirOn', false);
fl.rest      = util_htpDirListing(indir.rest,'ext','.set', 'subdirOn', false);

% import data
[results] = util_htpImportMea(indir.raw, 'nettype','MEA30', 'ext', '.edf', 'outputdir', outdir.rest, 'dryrun', false );

% Create output filenames
csv.basename = fullfile(outdir.rest, 'mea.csv');
...
%%  Spectral Power
res.rest.pow      = table();
[MeaCell, res.rest.pow] = runEegFun(loadEeg, runRest, getFiles(fl.rest), getPaths(fl.rest));
